# Archived decisions — 2026-07-17

Moved here by `/ds-project-compact`. Append-only, write-only history — not read by
`/ds-project-resume`. These entries are expired (shipped one-time choices) or
superseded; the live contract lives in `.project/DECISIONS.md`. DA2 (Linux
deferred) and DA5/DA10 (version from `package.json`) were later superseded by the
"Decouple version + add Linux" work shipped as `desktop-v1.0.0`.
